"""Defines a simple H-Type dipole in Radia"""

import radia as rad
import numpy as np

def _create_point_table(pole_width, pole_separation, pole_height, top_height, leg_width, gap_height, pole_face_n=50, pole_face_array=None):
    """
    ...
    if pole_face_array is not None:
        if type(pole_face_array) == list:
            pole_face_table = pole_face_array
        else:
            # assume pole_face_array is numpy array
            pole_face_table = pole_face_array.tolist()
    else:
        pole_face_table = []
        for i in np.linspace(pole_width, 0, pole_face_n):
            pole_face_table.append([i, gap_height])

    point_table = [p1, p2, p3, p4, p5, p6, p7] + pole_face_table
...
def make_dipole(pole_dimensions, center, length, current=-10000,
                trimesh_mode=0, longitudinal_divisions=4, pole_face_n=50, pole_face_array=None,
                triangle_min_size=0.5, triangle_max_size=1.0):
    """
    ...
    table_quadrant_one = _create_point_table(**pole_dimensions, pole_face_n=pole_face_n, pole_face_array=pole_face_array)
    top_coodinates = _get_all_points_top(table_quadrant_one)
    bottom_coordinates = _get_all_points_bottom(top_coodinates)

    top_pole = create_pole(top_coodinates, center, length, 
                           mode=trimesh_mode, triangle_min_size=triangle_min_size, triangle_max_size=triangle_max_size)
    bottom_pole = create_pole(bottom_coordinates, center, length, 
                              mode=trimesh_mode, triangle_min_size=triangle_min_size, triangle_max_size=triangle_max_size)
